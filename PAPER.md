...
3. **Two positive levers on the feasibility machinery**: faithful-collision stitching (+14pp) and trajectory-optimization repair (+8.9pp on GPD, +10.3pp on EDMP — planner-agnostic), the latter targeting and collapsing the diagnosed swept-collision failure (15.5→3.0%).
...
The stitcher originally certified bridges with the loose AABB intersection-volume *proxy*, which marks colliding bridges as free. Replacing it with a faithful PyBullet check inside RRT-Connect yields **+14pp** over the proxy plateau (62%→76% natural-weighted in earlier runs), the first intervention to move SR.
...
Two results: **densify-only ≈ base** (dynamic failures are genuine swept-collision, not PD overshoot), and **trajopt repair = +10.2pp** (cubby +20, dresser +10, merged +11), breaking the plateau that every prior-side change could not. The optimizer targets a densified static check; the benchmark independently verifies dynamic execution, so the 83% is a real executable-SR gain. Cost: ~+1.8s/scene, only on scenes that need repair (early-stop when edge-free).
...
| **+ trajopt repair** | **67.5** | 97 | **68** | **42** | **63** | 50.3s |

Repair lifts EDMP by **+10.3pp** (cubby +18, merged +12, dresser +10) — *larger* than the GPD gain (+8.9pp) and concentrated on the same constrained scene types. The lever is a property of the **inference-time feasibility machinery**, not of the GPD prior: it transfers across planners with different chains, representations, and guidance.
...
## 6. Conclusion
In a controlled study of guided polynomial diffusion planning, the generative prior is not the bottleneck: four natural prior improvements leave success rate flat while improving nominal metrics. The gains live in the feasibility machinery — faithful-collision stitching (+14pp) and trajectory-optimization repair (+10.2pp, to 83% on hybrid). We release the implementation and all ablations to support feasibility-centric, rather than prior-centric, research on diffusion planners.
