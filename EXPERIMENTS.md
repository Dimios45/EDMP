...
| D1 model | 20k | linear thr **0.08** | 0.433 | `models_d1/GPDModel64_N8/` |
...
# D1 (calibrated schedule, separate dir)
...
| `--guides "1"` | guide ids (GPD-1G = single guide) |
...
### 4b. Reproduction sprint — GPD-1G (1 guide, K=32, best-of-K, no stitch)
| Run | nat-wtd SR | per-type (tab/cub/merg/dres) |
|---|---|---|
| Exp 1 (200k, 300 scenes) | **62.3%** | 96/56/44/41 |
| 2a (20k, 150-scene subset) | 66.0%* | 100/72/28/48 |
| 2b D1 (20k, thr 0.08) | 53.3% | 84/44/16/46 |
| 12-guide + linear stitch (200k) | ≈53% | — |
| **Paper GPD-1G** | **72.9%** | — |

\*smaller/easier subset than Exp 1.

→ **GPD-1G reproduced to ~10 pp.** The earlier ~38 pp "gap" was mostly a **config artifact** (12-guide + linear stitch underperforms single-guide best-of-K). **D1 schedule fix refuted** (66.0→53.3, worse on all types despite lower loss; the low-noise ᾱ_T≈0.52 regime is an implicit smoothness prior — ε-loss misleads).

### 4c. D2 — stitching ablation (GPD-1G, 200k, 150-scene natural subset, 24 sharded workers)
...
- **But RRT only *ties* best-of-K overall (62%)** — it does NOT yet reproduce the paper's GPDS jump (72.9→92.8). It slightly underperforms on merged_cubby/dresser (within the ±~18 pp CI of n=25/type).
- **Why no GPDS-level gain (hypotheses → next work):** (i) RRT uses the AABB intersection-volume *proxy* as its collision checker, not true mesh/PyBullet collision — bridges valid under the proxy can still collide in reality; (ii) single-guide K=32 may lack the candidate *diversity* GPDS relies on; (iii) stitch-target heuristic (dist_threshold, earliest-forward) is crude. Next: a faithful collision checker inside RRT + higher candidate diversity.
...
### 4d. D5 — diversity × stitching (GPD, 200k, 150-scene natural)
...
### 4e. ⭐ D2-faithful — RRT + PyBullet collision checker (GPD, 200k, 150-scene natural)
| config | nat-wtd | tab | cub | merg | dres |
|---|---|---|---|---|---|
...
**⭐ The faithful collision checker breaks the 62% plateau → 76% (+14 pp).** The AABB intersection-volume proxy was the limiter — it certified bridges that actually collide in PyBullet. With true `getContactPoints` checking in RRT, gains land on the previously-weak constrained scenes: **dresser 38→66, merged_cubby 24→52, cubby 72→88.** Now **above the paper's GPD-1G (72.9%)**. Implemented via `RobotEnvironment.configs_free()` + `stitch(collision_fn=...)`; enable with `run_worker.py --pybullet_collision`. (n=25/type for cubby/merged → ±~18 pp CI; dresser n=50; gains large enough to be real. PyBullet check stays fast: ~0.1–0.3 s/bridge.)
...
| | global | hybrid | both |
|---|---|---|---|
| GPD-1G | 50.8 / 65.5 | 57.8 / 72.9 | 54.2 / 73.0 |
| GPD-7G | 56.4 / 81.9 | 63.8 / 90.9 | 64.0 / 90.2 |
| GPDS | 63.5 / 87.0 | 70.7 / 92.8 | 71.7 / 92.6 |

**Avg −21 pp.** Method + ordering (GPDS>7G>1G) replicate; absolute SR does not. Gap is ~half tunable (K=32→128 +6 pp; 2× guidance +4.5 pp) and ~half model-weights/guide-tuning. Faithful-collision RRT stitching is the proven +13–14 pp win; candidate-pool diversity / longer training / noise-schedule recalibration all tested and don't help.

## 5. Key findings
1. **Training duration is not the bottleneck** (+2.8 pp for 10×).
2. **GPD-1G reproduces to ~10 pp** with the correct single-guide best-of-K config.
3. **D1 (terminal-SNR recalibration) refuted** — low-noise schedule is an implicit smoothness prior; ε-loss ≠ planning success.
