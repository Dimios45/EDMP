#!/bin/bash
# B2: oracle-guided selection ablation. Compare guide-cost selection (existing
# seed_baseline_*/seed_repair_*) vs oracle-best-of-K ± trajopt repair. hybrid 400, 5 seeds.
set -e
...
LOG=logs/selection_ablation_main.log; mkdir -p logs results
...
for s in 0 1 2 3 4; do
    run oracle_norepair_s$s  --oracle_select                          --seed $((1000*(s+1)))
    run oracle_repair_s$s    --oracle_select --trajopt --trajopt_iters 60 --seed $((1000*(s+1)))
done
echo "===== ALL SELECTION-ABLATION DONE $(date) =====" | tee -a "$LOG"
