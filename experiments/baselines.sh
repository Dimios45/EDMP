#!/bin/bash
# Matched-split baseline table: GPD-base, GPD+D6 (trajopt), EDMP across
...
runp () {  # $1=tag  $2..=run_worker flags
    tag=$1; shift; rdir=results/base_$tag; mkdir -p "$rdir" logs
...
for ds in hybrid global both; do
    ...
    MTH=gpd; runp d6_${ds}   --method gpd --guides 1 --batch_per 32 --pybullet_collision --trajopt --trajopt_iters 60 --dataset $ds
