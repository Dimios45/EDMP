#!/bin/bash
# D3: scene-conditioned (CFG) GPD vs unconditional N8 baseline (71.8%).
# Sweep CFG weight; GPDS config (1 guide + faithful-PyBullet RRT stitch).
set -e
...
for w in 1.0 2.0 3.0; do
    tag=$(echo $w | tr '.' 'p'); rdir=results/d3_cfg$tag; mkdir -p "$rdir" logs
