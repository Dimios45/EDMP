...
\noindent\textbf{The audit generalizes across datasets and planners.}
Fig.~\ref{fig:auditcross}: the waypoint$\to$dense success gap is universal --- base GPD
loses $27/18/21\pp$ on global/hybrid/both, and EDMP loses $10\pp$ on hybrid --- and in
every case repair flattens the curve and dynamic execution tracks the dense static
check. This makes the audit a general evaluation-rigor prescription: learned planners
should report success at a stated continuous-check resolution (classical planners
already do \cite{rrtconnect2000}), not at the waypoints alone.
...
\subsection{Can the feasibility lever be shortcut? Two more negative levers}
\label{app:levers}
Beyond a denser objective, we tested two literature-motivated ways to improve/replace
the stitch$+$trajopt machinery. Both fail to beat it, reinforcing that it is the lever.
...
\noindent\textbf{Synthesis.} Across four attempts to improve or shortcut the feasibility
machinery --- denser continuous objective, exact sphere-SDF objective/guidance,
verifier-guided selection, and learned amortized repair --- none significantly exceeds
plain oracle-gated trajopt repair. The lever is robust; the returns are in \emph{using}
