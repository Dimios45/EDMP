...
### D3. Scene-conditioned denoiser (classifier-free / hybrid guidance) *(❌ REFUTED — conditioning does not raise the ceiling)*
...
- **❌ RESULT (GPDS, 100 sc/type hybrid):**
...
- **Verdict:** FOURTH instance of ε-loss ⊥ planning-SR decoupling (D1, D4-cap, D4-smooth, D3). The prior already has all it needs; scene info enters effectively via guidance, and adding it to the prior changes nothing.

### D4. Adaptive / piecewise polynomial representation *(❌ NAIVE VARIANT REFUTED — capacity backfires; see result below)*
- **❌ RESULT (N16 vs N8, both @20k, GPDS config, 100 sc/type hybrid):** overall **71.8% → 57.2% (−14.6pp)**; tabletop 99→98, cubby 68→56, **merged_cubby 61→28 (−33!)**, dresser 59→47. N16 worse everywhere, catastrophic on the most-constrained scene — the *opposite* of the hypothesis. Validity anchor: N8@20k GPDS (71.8%) ≈ N8@200k replication (70.7%) → budget fair.
- **Why it backfired:** the reconstruction diagnostic (below) proved headroom exists *in the demos*, but the diffusion prior cannot exploit it — extra control points produce jaggier, more-colliding samples; **losing the low-degree smoothness regularizer dominates the representational gain.** Same lesson as D1: GPD's compactness is an implicit smoothness prior, and nominal loss / reconstruction-fidelity are anti-correlated with planning SR.
...
- **FINAL VERDICT (3 experiments agree):** added representational capacity is a net loss for planning SR despite 4× lower reconstruction error + equal train loss. Reconstruction fidelity is **decoupled from** planning SR. With D1 (schedule) this triangulates the thesis: **GPD's compactness is an implicit, well-conditioned regularizer; the binding constraint is INFORMATION (environment conditioning), not capacity.** → D3 is the sole remaining ceiling-raiser.
...
- **Experiment:** N16 vs N8(@20k) on hybrid, GPDS config (1 guide + faithful-PyBullet RRT stitch), per-type SR — watch cubby/merged_cubby/dresser.
...
### D6. Continuous-feasibility trajectory-optimization repair *(✅✅✅ VALIDATED — +10.2pp, breaks the plateau)*
...
- **✅ RESULT (100 sc/type hybrid, GPDS):**
  | config | overall | tab/cub/merg/dres |
  |---|---|---|
  ...
  - **Trajopt repair = +10.2pp** (cubby +20, dresser +10, merged +11). The **first** lever besides faithful stitching to move SR, and it breaks the ~72% plateau that resisted D1/D3/D4. Cost: +~1.8s/scene, only on failing scenes (early-stops when edge-free).
